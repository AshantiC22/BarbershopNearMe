...
export default function LoadingScreen({ children }){
  ...
  useEffect(()=>{
    ...
    function frame(ts){
      ...
      const gY=H-78;
      const catBounce  =Math.pow(Math.abs(sin(t,14,1)),.6)*7;
      const mouseBounce=Math.pow(Math.abs(sin(t,16,1)),.6)*5.5;
  ...
  const handleEnter=()=>{setPhase('exit');setTimeout(()=>setExit(true),520);};
