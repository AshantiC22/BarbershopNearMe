...
.ls-root{position:fixed;inset:0;z-index:9999;background:#050403;display:flex;align-items:center;justify-content:center;overflow:hidden;transition:opacity .5s ease,transform .5s ease;}
.ls-root.exit{opacity:0;transform:scale(.96) translateZ(0);pointer-events:none;transition:opacity .55s cubic-bezier(.16,1,.3,1),transform .55s cubic-bezier(.16,1,.3,1);}
...
.ls-canvas-zone{position:relative;height:400px;background:#050403;overflow:hidden;}
...
/* TITLE */
.ls-title-overlay{position:absolute;inset:0;z-index:20;background:#050403;display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity .42s ease;}
...
.ls-title-main{font-family:'Bebas Neue',sans-serif;font-size:clamp(54px,12vw,94px);line-height:.86;text-align:center;text-transform:uppercase;color:#E8DFC8;text-shadow:4px 4px 0 #8B1A1A;animation:ls-titlePop .65s cubic-bezier(.34,1.56,.64,1) both;}
@keyframes ls-titlePop{0%{transform:scaleY(2.5) scaleX(.55) rotate(-4deg);opacity:0;}100%{transform:none;opacity:1;}}
.ls-title-red{color:#8B1A1A;display:block;text-align:center;font-size:1.25em;line-height:1.1;animation:ls-redPulse 2.2s ease-in-out .65s infinite;}
...
/* DONE */
.ls-done{position:absolute;inset:0;z-index:20;background:#050403;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;opacity:0;pointer-events:none;transition:opacity .55s ease;}
...
export default function BookNowTransition({ onDone }){
  ...
  const linesRef = useRef(
    Array.from({length:13},()=>({
      y:10+Math.random()*340, speed:280+Math.random()*230,
  ...
  useEffect(()=>{
    ...
    function resize(){
      ...
      const tmpCtx=cv.getContext('2d');
      if(tmpCtx){tmpCtx.imageSmoothingEnabled=false;}
  ...
  useEffect(()=>{
    ...
    function frame(ts){
      ...
      const dt=Math.min((ts-lastRef.current)/1000,.033); /* cap at 30fps min */
      lastRef.current=ts;
      tRef.current+=dt;
      ...
      if(splatTRef.current<=0&&p>8){splatTRef.current=1.9+Math.random()*1.4;spawnSplat();}
      ...
      const gY=H-78;
      const razorBounce=Math.pow(Math.abs(sin(t,16,1)),.6)*7;
      const curlBounce =Math.pow(Math.abs(sin(t,18,1)),.6)*5.5;
  ...
  const handleEnter=()=>{setPhase('exit');setTimeout(()=>{setExit(true);onDone?.();},520);};
            ...
            <div className="ls-canvas-zone">
              ...
              <div className="ls-title-overlay" ref={titleRef}>
                ...
                <div className="ls-title-main">
                  The
                  <span className="ls-title-red">Chair</span>
                  Awaits
                </div>
                <div className="ls-title-sub">— Razz &amp; Curl — A Close Shave —</div>
              ...
              <div className="ls-done" ref={doneRef}>
                <div className="ls-done-title">Ready to<br/>Book.</div>
