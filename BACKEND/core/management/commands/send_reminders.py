            ...
            if appt_dt < stale_cutoff:
                ...
                try:
                    ...
                    if barber_email:
                        ...
                        _sendgrid_send(
                            barber_email,
                            f"🚫 No-Show: {client_nm} — Barbershopnearme",
                            plain,
...
<tr><td style="padding-bottom:20px;">
  <p style="font-family:'Courier New',monospace;font-size:20px;font-weight:900;text-transform:uppercase;">Barbershopnearme</p>
</td></tr>
...
<tr><td style="padding-bottom:20px;">
  <p style="color:#71717a;font-size:13px;line-height:1.7;">
    <strong style="color:white;">{client_nm}</strong> did not show up for their {svc} at <strong style="color:#8B1A1A;">{t_str}</strong>.
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
        ...
        <tr><td style="padding-bottom:24px;">
          <p style="color:#71717a;font-size:13px;margin:0;line-height:1.7;">
            Hey {waiting.client_name}! A slot just opened up on <strong style="color:#8B1A1A;">{date_str}</strong> — book it now before someone else does.
          </p>
        </td></tr>
        ...
        <tr><td style="border-top:1px solid rgba(255,255,255,0.06);padding-top:20px;margin-top:20px;">
          <p style="font-size:11px;color:#3f3f46;margin:0;">Barbershopnearme · 910 W Parker Rd Bld 300, Plano, TX 75023</p>
                    ...
                    _sendgrid_send(
                        ...
                        f"✂️ A Slot Opened Up — {date_str} — Barbershopnearme",
                        plain, html
                    )
        ...
        for appt in appts:
            ...
            try:
                if not appt.reminder_sent and 23 <= diff_hours <= 25:
                    if client_email:
                        subj  = "⏰ Reminder: Your appointment tomorrow at Barbershopnearme"
                        plain = (
                            f"Hey {client_nm},\n\n"
                        ...
                        html = _build_client_html(
                            ...
                            "Your appointment is tomorrow", "#8B1A1A", "⏰"
                        )
            ...
            try:
                ...
                if not reminder_2hr and 1.5 <= diff_hours <= 2.5:
                    if client_email:
                        subj  = f"✂️ Your appointment is in 2 hours — Barbershopnearme"
                        plain = (
                            f"Hey {client_nm},\n\n"
                        ...
                        if client_phone:
                            _twilio_send(client_phone,
                                f"✂️ Barbershopnearme: 2 hours away!\n"
                ...
                if not barber_2hr and 1.5 <= diff_hours <= 2.5:
                    if barber_email:
                        ...
                        html = _build_barber_html(
                            ...
                            f"{client_nm} arriving in 2 hours — get ready!", "#8B1A1A", "⚡"
                        )
                        _sendgrid_send(barber_email, subj, plain, html)
                        if barber_phone:
                            _twilio_send(barber_phone,
                                f"⚡ Barbershopnearme: {client_nm} in 2 hours!\n"
            ...
            try:
                ...
                if not barber_now and -0.1 <= diff_hours <= 0.2:
                    if barber_email:
                        ...
                        if barber_phone:
                            _twilio_send(barber_phone,
                                f"🪑 Barbershopnearme: {client_nm} is up NOW!\n"
...
def _build_barber_html(barber, client, service, date, time, notes, headline, color, icon):
    notes_row = f"""
        <tr>
          ...
          <td style="padding:10px 0;font-size:13px;color:#8B1A1A;text-align:right;font-style:italic;">"{notes}"</td>
        </tr>""" if notes else ""
    return f"""
<div style="background:#000;color:#fff;font-family:'Helvetica Neue',Arial,sans-serif;max-width:520px;margin:0 auto;">
  <div style="background:linear-gradient(to right,{color},{color}99);height:3px;"></div>
  <div style="padding:32px 28px;">
    <p style="font-size:11px;letter-spacing:0.5em;text-transform:uppercase;color:rgba(245,158,11,0.7);margin:0 0 8px;">Barbershopnearme · BARBER ALERT</p>
    <h1 style="font-size:22px;font-weight:900;text-transform:uppercase;letter-spacing:-0.02em;margin:0 0 24px;color:#fff;">
      {icon} {headline}
    </h1>
    <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);padding:20px;margin:0 0 20px;">
