...
const T = {
  ...
  blood: '#8B1A1A',
  blood2: '#6B0F0F',
  dim1: 'rgba(232,223,200,.75)',
  dim2: 'rgba(232,223,200,.22)',
}
...
export default function ClientReschedulePage() {
  ...
  useEffect(() => {
    ...
    const barberId = appt?.barber?.id || appt?.barber_id
    if (!barberId) return
    setSlotsLoading(true)
    setSelectedTime(null)
    const dateStr = selectedDate.toISOString().split('T')[0]
    api
      .get(`available-slots/?barber=${appt.barber?.id || appt.barber_id}&date=${dateStr}`)
      .then((d) => {
        const list = Array.isArray(d) ? d : d.available_slots || []
        setSlots(list)
      })
      .catch(() => setSlots([]))
      .finally(() => setSlotsLoading(false))
  }, [selectedDate, appt]) // eslint-disable-line
    ...
    return (
      ...
      >
        ...
        <p style={{ ...mono, fontSize: 13, color: T.dim1, maxWidth: 340, lineHeight: 1.7 }}>
          Your barber will review and approve or decline your request. You'll get a notification
          either way.
        </p>
        <button
  ...
  const isDisabled = (day) => {
    ...
    d.setHours(0, 0, 0, 0)
    // Disable today and past — must book at least 1 day ahead
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <button
            ...
            style={{
              ...
              opacity: !selectedDate || !selectedTime || submitting ? 0.6 : 1,
              boxShadow: `4px 4px 0 ${T.bone}`,
              transition: 'all .2s',
            }}
          >
