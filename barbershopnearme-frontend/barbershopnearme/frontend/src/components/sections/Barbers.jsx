...
function BarberCard({ barber, index, isFallback, onSelect }) {
  ...
  return (
    ...
    >
      ...
      {/* spec pill */}
      <span className="barber-spec">{isFallback ? barber.spec : 'Book Now →'}</span>
    </div>
  )
}
...
export default function Barbers() {
  ...
  /* After data loads, manually trigger .in on all .reveal elements
     that are already visible in the viewport */
  useEffect(() => {
    if (loading) return
      ...
      <div className="container">
        ...
        <div className="section-intro">
          <h2 className="t-title reveal" style={{ transitionDelay: '.06s' }}>
            {headlineNum} Blade{showLive && barbers.length === 1 ? '.' : 's.'}
            <br />
          ...
          <div className="barber-grid">
            {[0, 1, 2].map((i) => (
              <div
                ...
                style={{
                  background:
                    'linear-gradient(90deg,rgba(232,223,200,.02) 25%,rgba(232,223,200,.06) 50%,rgba(232,223,200,.02) 75%)',
